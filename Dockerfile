...
# Install dependencies to add PPAs and X11 development libraries
RUN apt-get update && \
...
RUN apt-get update && \
    apt-get install -y build-essential python-dev python3-dev python3.9-distutils python3.9-dev python3.9 curl supervisor && \
    apt-get clean && \
